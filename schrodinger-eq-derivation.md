...
And the known solutions to it are $$\Psi(x,t)=Ae^{i(kx-\omega t)}$$
...
where $p$ is the momentum and $c$ is the speed of light. In out wave-equation $k=\frac{2\pi}{\lambda}$ is the wavenumber, $\omega=2\pi\nu$ is the frequency. 
...
$$\frac{\partial^2 \Psi}{\partial t^2} = {\frac{-iE}{\hbar}}^2\Psi$$
$$\frac{\partial^2 \Psi}{\partial x^2} = {\frac{iP}{\hbar}}^2\Psi$$
substituting in:
$${\frac{-iE}{\hbar}}^2\Psi=c^2{\frac{iP}{\hbar}}^2\Psi$$
$${E}^2=c^2{P}^2$$
...
Let's take the secons position derivative:
...
Nice! This is the schrodinger equation in 1D, If we were to extrapolate this into 3D we would get the previously mentioned schrodinger equation with the lapalcian operator in place of a single second derivative. 
...
This ensure that the probability across space is always $<1$. This also limits the allowed wavefunction to only square integrable functions. 
